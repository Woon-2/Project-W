...
## [feat] (temp_server): 클라-서버 시간 동기화 개선
...
## 2026-06-22 — 전술 전투 피해 프로파일 개선

- 공용 전술 기반 클래스에서 보스와 전 부대원의 `damageTakenMultiplier`를 phase별로 일괄 관리한다.
- 홉고블린은 전술 해금 후 후퇴·박스 준비 중 전체 무적, 실제 포위/쐐기 시작 시 정상 피해로 복구한다.
- 이시스는 후퇴부터 2차 쐐기 종료까지 전체 무적이며, 부대 전멸 시 진행 중 전술을 취소하고 `Engage`를 보스 단독 전투로 재사용한다.
- 그랜드밤은 평상시 슬라임 무적, ShieldWall 중 보스 90%·슬라임 50% 피해 경감을 적용한다.
- 모든 전술에서 보스가 먼저 죽으면 생존 부대원의 피해 경감을 해제하고 Confused 상태로 전환한다.
- HP가 실제 감소하지 않은 타격은 최근 공격자와 킬 충전 기여도로 기록하지 않는다.
