...
void GoblinMidBossTactic::evaluateTactics( Room& room, PlatoonLeader& leader ) {
    ...
    if ( leaderPhase_ == LeaderPhase::TacticalRetreat ) {
        ...
        if ( awayDir.len2() < 0.001f ) awayDir = mu::Vec3( -1.f, 0.f, 0.f );
    ...
    if (leaderPhase_ == LeaderPhase::BoxAdvance) {
        ...
        for (auto* sq : liveSquads) {
            mu::Vec3 sum{}; int cnt = 0;
            for (TacticalNpc* npc : sq->getMemberCache()) {
                if (npc && npc->hp() > 0) { sum = sum + npc->pos(); ++cnt; }
            }
    ...
    const std::vector<TacticalSquad*>& liveSquads) const
{
    ...
    for (const TacticalSquad* sq : liveSquads)
        if (sq) total += static_cast<int>(sq->getMembers().size());
    return total;
}

int GoblinMidBossTactic::minMembersForEncircle(int32 playerCount) const {
...
float GoblinMidBossTactic::calcEncircleRadius(int32 liveMembers) const {
    constexpr float TWO_PI = 2.f * 3.14159265f;
    float radiusByCount = static_cast<float>(std::max(liveMembers, 1)) *
...
bool GoblinMidBossTactic::checkTacticsConditions(const PlatoonLeader& leader) const {
    ...
    if (maxHp > 0.f &&
        static_cast<float>(leader.hp()) / maxHp <= TACTIC_HP_THRESHOLD)
        return true;
...
GoblinMidBossTactic::buildPlayerClusters(const Room& room,
    ...
    for (PlayerCluster& cluster : result) {
        ...
        for (uint32_t id : cluster.playerIds) {
            const GameSession* s =
                room.findLivingSessionByPlayerId(static_cast<int32>(id));
            if (!s) continue;
...
GameSession* GoblinMidBossTactic::selectPrimaryTarget(
    ...
    const std::vector<uint32_t>& playerIds) const
{
    ...
    return best ? static_cast<uint32_t>(best->id()) : 0;
}
    ...
    const std::vector<PlayerCluster>& clusters)
{
    ...
    for (int i = 0; i < static_cast<int>(liveSquads.size()); ++i) {
        float d = (liveSquads[static_cast<size_t>(i)]->calcCentroid() -
                   chargeCluster.centroid).len();
        if (bestDist < 0.f || d < bestDist) { bestDist = d; chargeSquadIdx = i; }
    }
    ...
    for (size_t i = 1; i < sorted.size(); ++i) {
        ...
        ++supportCount;
        if (supportTargetId == 0) supportTargetId = cl.representativeId;
        supportPlayerIds.insert(supportPlayerIds.end(),
                                cl.playerIds.begin(), cl.playerIds.end());
    }
    ...
    if (blockDir.len2() < 0.001f) blockDir = mu::Vec3(1.f, 0.f, 0.f);

    mu::Vec3 blockCenter = chargeCluster.centroid +
...
void GoblinMidBossTactic::updateDivideAndConquer(float dt, Room& room,
    ...
    for (auto& task : divideTasks_) {
        if (task.taskCompleted && !task.engageIssued) {
            ...
            uint32_t targetId = selectReplacementTarget(room, leader, task.clusterPlayerIds);
        ...
        if (task.engageIssued) {
            if (task.targetId != 0) {
                GameSession* target =
                    room.findLivingSessionByPlayerId(static_cast<int32>(task.targetId));
                if (!target) {
    ...
    float dt, Room& room, PlatoonLeader& leader)
{
        ...
        if (bossTargetEvalTimer_ <= 0.f) {
            ...
            if (candidate.targetId != 0 &&
                ...
                calcBossPersonalTargetScore(room, leader,
                                            bossPersonalTargetId_, currentScore) &&
    ...
    if (bossPersonalState_ == BossPersonalState::AttackWindup) {
        ...
        if (bossPersonalTimer_ < cfg.attackWindupTime) return true;
    ...
    if (bossPersonalState_ == BossPersonalState::AttackRecover) {
        ...
        bossPersonalTimer_ += dt;
        TacticalNpcConfig cfg = leader.getConfig();
        if (bossPersonalTimer_ < cfg.attackRecoverTime) return true;
    ...
    Room& room, const PlatoonLeader& leader) const
{
    ...
    for (const PlayerCluster& cluster : clusters) {
        ...
        for (uint32_t playerId : cluster.playerIds) {
            const GameSession* s =
                room.findLivingSessionByPlayerId(static_cast<int32>(playerId));
            if (!s) continue;
...
bool GoblinMidBossTactic::calcBossPersonalTargetScore(
    Room& room, const PlatoonLeader& leader,
    uint32 targetId, float& outScore) const
{
    ...
    for (const PlayerCluster& cluster : clusters) {
        if (std::find(cluster.playerIds.begin(), cluster.playerIds.end(), targetId) ==
            cluster.playerIds.end())
            continue;
        float distance = (leaderPos - s->player()->pos()).len();
...
GameSession* GoblinMidBossTactic::resolveBossPersonalTarget(
    Room& room, uint32 targetId) const
{
    if (targetId == 0) return nullptr;
    return room.findLivingSessionByPlayerId(static_cast<int32>(targetId));
}

void GoblinMidBossTactic::moveBossToward(PlatoonLeader& leader, mu::Vec3 targetPos,
...
float GoblinMidBossTactic::evaluatePlayerScore(
    const GameSession* s, const PlatoonLeader& leader) const
{
...
std::vector<mu::Vec3> GoblinMidBossTactic::calcSquadBoxOffsets(int32 numSquads) const {
    int rows = static_cast<int>(
        std::max(1.f, std::floorf(std::sqrtf(static_cast<float>(numSquads)))));
