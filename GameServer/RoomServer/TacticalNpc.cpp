...
void TacticalNpc::updateReservedAttackStaleTimer(float dt, Room& room) {
    ...
    if (state_ == TacticalNpcState::AttackWindup ||
        state_ == TacticalNpcState::AttackRecover ||
        distSq <= attackRange_ * attackRange_) {
        reservedAttackStaleTimer_ = 0.f;
        return;
    }
...
void TacticalNpc::moveTowardPressureWait(float dt, Room& room,
                                          mu::Vec3 targetPos, mu::Vec3 targetFacing) {
    ...
    bool targetMoved =
        (targetPos - pressureWaitTargetAnchor_).len2() >
        TACTICAL_PRESSURE_TARGET_MOVE_REFRESH_DIST * TACTICAL_PRESSURE_TARGET_MOVE_REFRESH_DIST;
    bool facingChanged =
        pressureWaitFacingAnchor_.len2() <= 0.01f ||
        mu::dot(tf, pressureWaitFacingAnchor_) < TACTICAL_PRESSURE_FACING_REFRESH_DOT;
    ...
    // 분리력 (접근 방향 수직 성분만)
    nearbyCache_.clear();
    float sepRad = std::max(separationRadius_,
                            separationRadius_ * TACTICAL_PRESSURE_SEPARATION_RADIUS_MULT);
    ...
    if (sep.len2() > 0.01f && moveDir.len2() > 0.01f) {
        ...
        float nearScale     = std::max(TACTICAL_PRESSURE_NEAR_SEPARATION_MIN_SCALE,
            std::min(1.f, distToDesired / TACTICAL_PRESSURE_SLOW_RADIUS));
        moveDir = norm3(moveDir + sepPerp *
            (separationWeight_ * TACTICAL_PRESSURE_SEPARATION_WEIGHT_MULT * sepScale * nearScale));
    }
    ...
    nearbyCache_.clear();
    float overlapRad = std::max(TACTICAL_PRESSURE_OVERLAP_RADIUS_MIN,
    ...
    if (overlapSep.len2() > 0.0001f) {
        ...
        if (radial.len2() <= 0.01f)
            radial = mu::Vec3((pressureWaitDesired_ - targetPos).x(), 0.f,
                              (pressureWaitDesired_ - targetPos).z());
        if (radial.len2() > 0.01f) {
            radial              = norm3(radial);
...
void TacticalNpc::updateAttackRecover(float dt, Room& room) {
    ...
    if (recoverTimer_ >= attackRecoverTime_) {
        if ((pos() - target->player()->pos()).len2() <= attackRange_ * attackRange_ &&
...
void TacticalNpc::updatePressureWait(float dt, Room& room) {
    ...
    float reenterDelay = TACTICAL_PRESSURE_REENTER_MIN_TIME +
        static_cast<float>(getId() % 4u) * TACTICAL_PRESSURE_REENTER_STAGGER;
...
void TacticalNpc::updateChargeThrough(float /*dt*/, Room& room) {
    ...
    if (chargeComplete_) {
        if (target) setFacingDir(*this, target->player()->pos() - pos());
        setLinearVel(mu::Vec3(0.f, body().linearVel().y(), 0.f));
        return;
    }
    ...
    for (GameSession* s : room.getLivingPlayers()) {
        if (!s) continue;
        if ((pos() - s->player()->pos()).len2() > impactRSq) continue;
...
void TacticalNpc::updateConfused(float dt, Room& room) {
    ...
    if (confusedRetargetTimer_ <= 0.f || distToTarget < 0.75f) {
        ...
        float seedA = static_cast<float>(
            (getId() * 37u + static_cast<uint32_t>(confusedWanderStep_) * 101u) % 997u);
        float seedB = static_cast<float>(
            (getId() * 53u + static_cast<uint32_t>(confusedWanderStep_) * 193u) % 991u);
        ...
        confusedTarget_         = confusedAnchor_ +
            mu::Vec3(std::cosf(angle) * radius, 0.f, std::sinf(angle) * radius);
