...
private:
    ...
    std::vector<mu::Vec3> MU_CALLCONV calcEncircleSlots(
        mu::Vec3 targetPos, float sectorAngle, float sectorSpan,
        float radius, int count) const;
    std::vector<mu::Vec3> MU_CALLCONV calcDenseSlots(
        mu::Vec3 center, mu::Vec3 forward, int count,
        float spacingScale = 1.f, float columnScale = 1.f,
        int fixedColumnCount = 0) const;
    std::vector<mu::Vec3> MU_CALLCONV calcWedgeSlots(
