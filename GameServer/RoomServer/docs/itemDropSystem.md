...
| 드롭이 **화면에 나타나는 시점** | **클라** (사망이 아니라 에너지 오브 생성 시점) |
...
**드롭 등장 타이밍은 사망이 아니라 에너지 오브 생성에 맞춘다.** 서버 통지(`S_ItemDropBatch`)는
사망 즉시 오지만, 래그돌이 널브러진 채로 보석이 튀어나오면 인과가 어긋난다. 그래서 클라는
통지를 `pendingGemDrops_`에 담아뒀다가 해당 시체가 `Corpse::Phase::Orb`로 전환될 때
(`kRagdollSeconds = 2.0s` 후) `releasePendingItemDrops(origId, corpsePos)`로 꺼내 스폰한다.
대응 시체를 못 찾는 경우(`sourceObjId == 0`, 사망 순간을 못 본 몬스터)를 위해
`kPendingDropTimeoutSec = 3.0s` 강제 스폰 시한이 있다 — 없으면 **서버엔 있는데 클라엔 영영
안 보이는 드롭**이 생긴다. 이 지연 동안 서버는 이미 습득 가능 상태지만, 클라가 조준할 수
없으므로 실질적으로 습득도 불가능하다(TTL 180s 대비 무시할 수준).
