...
// Minimum vertical clearance a camera sample must keep above terrain.
// (Was PhysicsWorld::kCameraMinGroundClearance; moved here with queryArm.)
...
{
    // Identity-rotation test: rotate two basis vectors; if both are unchanged the
    // transform is pure scale+translate, so an AABB can stay an AABB (cheaper).
    ...
    return std::visit([&](auto&& s) -> std::variant<AABB, OBB> {
        ...
        if constexpr (std::is_same_v<T, AABB>) {
            ...
            // Rotated instance: an axis-aligned model box becomes a world OBB.
...
bool TerrainCollider::testVertex(mu::Vec3 worldVert, ContactPoint& outCp, float lookAhead) const
{
    ...
    // Also accept vertices within lookAhead above the surface (free-fall snap).
    if (depth <= -lookAhead) return false;

    outCp.worldPos = mu::Vec3(worldVert.x(), terrainWorldY, worldVert.z());
    // Always use Y-up: slope-perpendicular normal would apply a tilting torque to the body.
...
int TerrainCollider::generateContacts(RigidBody& dyn, const ContactSink& sink) const
{
    ...
    // Free-fall snap: for a falling body extend the contact range by one sub-step's
    // worth of travel so the constraint catches it before it tunnels.
...
float TerrainCollider::queryArm(mu::Vec3 pivot, mu::Vec3 dir, float armLen, float /*spherePad*/) const
{
    if (!heightField_ || heightField_->empty()) return armLen;
    float allowed = armLen;
    constexpr int N = 6;
    const mu::Vec3 origin = terrainBody_->pos();
    for (int i = 1; i <= N; ++i) {
        const float t = static_cast<float>(i) / N;
        const mu::Vec3 p = pivot + dir * (t * armLen);
        const float lx = p.x() - origin.x();
        const float lz = p.z() - origin.z();
        if (lx < 0.f || lx > heightField_->sizeX) continue;
        if (lz < 0.f || lz > heightField_->sizeZ) continue;
        const float groundY = origin.y() + heightField_->getHeightAt(lx, lz);
...
ScatterCollider::ScatterCollider(const std::vector<InstanceInput>& instances)
{
    ...
    for (const auto& in : instances) {
        ...
        // worldAABB = union of all node bounds.
        bool     first = true;
    ...
    // Build the XZ uniform grid (static instances bucketed once).
...
float ScatterCollider::queryArm(mu::Vec3 pivot, mu::Vec3 dir, float armLen, float spherePad) const
{
    if (insts_.empty()) return armLen;
    float allowed = armLen;

    const mu::Vec3 eye = pivot + dir * armLen;
    const mu::Vec3 mn  = mu::min(pivot, eye) - mu::Vec3(spherePad, spherePad, spherePad);
    const mu::Vec3 mx  = mu::max(pivot, eye) + mu::Vec3(spherePad, spherePad, spherePad);
    const AABB armAABB{ (mn + mx) * 0.5f, mx - mn };
    const Ray  ray{ pivot, dir };

    forEachCandidate(armAABB, [&](int i) {
        if (!collides(armAABB, insts_[i].worldAABB).hit) return;
        const RayHit h = RaycastBVH(insts_[i].worldBVH, ray);
        if (h.hit) allowed = std::min(allowed, h.t - spherePad);
    });
