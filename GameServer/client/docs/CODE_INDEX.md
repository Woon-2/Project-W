...
| 디스플레이 설정 런타임 변경 (해상도 + 전체화면) | `online/onlineGame.cpp` (`applyPendingDisplaySettings`/`applyDisplaySettings`), `gfx.cpp` (`GFX::resize`), `main.cpp` (`applyDisplayMode`), `sharedResources.cpp` (`GBuffer::eraseGBuffer`), `ui/UIManager.cpp` (`resetInteractionState`) | `update()` 진입부에서 `settings_.resolutionIndex`(0=1024×768/1=1280×720/2=1920×1080)·`fullscreen` 변화를 감지해 **프레임 안전 지점**에서 적용(버튼 콜백 내 재빌드 금지 → dangling 방지). 전체화면은 **borderless**(`WS_POPUP`+모니터 전체, `applyDisplayMode`) — exclusive 아님, 스왑체인은 windowed 유지. `GFX::resize`: GPU idle→백버퍼/깊이/GBuffer/HiZ 해제(풀 슬롯 반납)→`ResizeBuffers`→재생성(뷰포트는 `gClientRect` 자동 추종). `LobbyUI/SettingsPanel::build` 멱등(재빌드). 해상도 컨트롤은 창모드에서만 활성. 상세: `docs/lobbyUISeparation.md` |
