...
| `Game::lobbyLogin/Register/CreateRoom/JoinRoom/LeaveRoom/StartGame()` | `onlineGame.cpp` (UI 버튼 콜백) | 로그인/회원가입은 **요청만 전송**하고 상태 전환은 응답 핸들러에서 한다. 로컬 검사는 빈 값·길이(`kLoginIdMax-1`/`kPasswordMax-1`/`kNicknameMax-1`)·ASCII 여부까지만 — 아이디/비밀번호는 wire 상 `char[]`라 비ASCII면 보내지 않고 거절(닉네임은 `wchar_t[]`라 한글 가능). 중복 판정은 전부 서버(DB UNIQUE). `authPending_`이 응답 대기 중 재전송을 막는다. Create/Join은 인증 상태를 이중 검사한 뒤 LobbyServer 요청 패킷 전송. 룸 상태 변경은 응답 핸들러에서 수행하며 Leave 후 인증 상태는 유지 |
| `Game::onRegisterResult/onLoginResult()` | `onlineGame.cpp` (public) | `S_Register`/`S_Login` 처리. `authPending_` 해제 후 `AccountResult`를 `accountResultMessage()`(파일 내 익명 네임스페이스)로 한국어 안내 문구에 매핑해 `setSignupMessage`/`setMainMenuMessage`로 표출. 성공 시 `onLoginResult`가 `accountId_`/`myNickname_` 저장 + `isAuthenticated_=true` + 비밀번호 칸 삭제 + `refreshLobbyUI()`, `onRegisterResult`는 `pendingRegisterId_`로 `completeRegistration()` 호출(가입 응답에 loginId가 없어 보낸 값을 들고 있어야 함) |
| `Game::onLobbyCreated/onLobbyJoined/onLobbyPlayerJoined/onLobbyPlayerLeft/onGameStart()` | `onlineGame.cpp` (lobby 액션 직후, public) | LobbyServer 응답 처리. `PacketManager`가 `LobbyScene`의 `SleepEx(1,true)` alertable 대기에서 호출. 룸 상태/슬롯/호스트 갱신 후 `refreshLobbyUI()`. `onGameStart(ip, port, code, ticket)`은 recv APC 안이라 소켓을 건드리지 않고 `handoffIp_/Port_/Code_/Ticket_` + `pendingHandoff_`만 적재 — 실제 재접속은 `LobbyScene`이 APC 밖에서 수행 |
| 계정 패킷 | `protocol.hpp` `CRegisterPacket`/`SRegisterPacket`/`CLoginPacket`/`SLoginPacket`, `AccountResult`; 클라 `PacketManager::makeCRegisterPacket/makeCLoginPacket` → `handleSRegisterPacket/handleSLoginPacket` → `Game::onRegisterResult/onLoginResult`; 서버 `LobbyServer/PacketManager.cpp` | 로비서버 인증. **로그인 성공 전에는 다른 `C_*`가 서버에서 조용히 폐기**되므로(인증 게이트) 방 생성/참가는 반드시 로그인 후. 고정 배열은 `BufferWriter::reserve`가 0 초기화를 하지 않아 make* 쪽에서 `memset` 후 복사(널 종료 요구 + 이전 전송의 비밀번호 잔재 유출 방지). 상세: `ServerEngine/docs/accountSystem.md` |
...
| `Game::lobbyDisplayName(uint16, const wchar_t*)` | `onlineGame.cpp` | sessionId+닉네임 → 표시 이름. 본인(`myLobbyId_`)은 계속 `"나"`(4칸 대기실에서 자기 슬롯 식별이 우선), 그 외는 `LobbyPlayerInfo::nickname`. 서버가 못 채운 경우에만 `"Player_<id>"`로 폴백 |
| 입장 티켓 핸드오프 | `protocol.hpp` `EntryTicket`/`EntryTicketPayload`, `SGameStartPacket::ticket`, `CEnterPacket::ticket`; 클라 `handleSGameStartPacket`→`Game::onGameStart`→`handoffTicket_`→`makeCEnterPacket(ticket, weapon)` | 로비/룸 사이에 소켓이 없어 클라가 계정 정보를 중계하므로 로비가 HMAC-SHA256으로 서명한다. 클라는 **티켓을 그대로 전달만** 하며 내용을 만들거나 고치지 않는다. `CEnterPacket`에 방 코드 사본이 따로 없다(권위 출처는 `ticket.payload.lobbyCode`). 상세: `ServerEngine/docs/entryTicket.md` |
...
| `Game` 통합 | `online/onlineGame.cpp` | `enterLobby`: `lobbyUI_.loadTextures/build` + `settingsPanel_.build`. `refreshLobbyUI()`는 씬/세션/인증 상태로 `LobbyUI::ViewState` 스냅샷을 만들어 `lobbyUI_.refresh()`에 위임(+메인메뉴 이탈 시 `settingsPanel_.close()`). `makeLobbyCallbacks()`가 로그인·회원가입(아이디/비밀번호/닉네임)·방 버튼 액션을 `lobbyLogin/lobbyRegister/lobbyCreateRoom` 등에 연결. 인증은 LobbyServer `C_Login`/`C_Register` 왕복으로 처리되고 계정은 DB에 영속된다. `LobbyScene`/`renderWaitingRoom`/`enterInGame`/`lobbyLeaveRoom`은 컴포넌트 메서드 호출 |
...
| `Game::register/unregisterInGamePartyPlayer()`, `partyDisplayName()` | `online/onlineGame.cpp` | 파티 로스터 + **등록 시점 고정 이름**(`inGamePartyNameById_`). 이름은 `registerInGamePartyPlayer(id, nickname)`의 계정 닉네임이 우선이고, 없을 때만 `"playerN"` 폴백. 인덱스 기반 이름은 퇴장 시 번호가 밀려 클라 간 표시가 어긋나므로 등록 시 1회 부여 후 불변 |
| `Game::prepareInGamePartyRoster(const PlayerInfo&, const std::vector<PlayerNameInfo>&)` | `online/onlineGame.cpp` | S_Enter 처리 시 로스터를 리셋하고 **닉네임을 먼저 심는다**. `handleSEnterPacket`이 objList 루프보다 앞서 호출하는 것이 중요 — 뒤이은 `createOtherPlayer(ObjectInfo)`에는 닉네임이 없고, 등록은 이미 있는 id에 대해 no-op이라 순서가 뒤바뀌면 닉네임이 `"playerN"`으로 덮인다. 기존 플레이어 닉네임은 `SEnterPacket`의 `PlayerNameInfo` 섹션으로 온다(`ObjectInfo`는 NPC와 공유하므로 넣지 않음) |
