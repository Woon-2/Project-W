...
- 온라인 인벤토리는 `RoomServer`의 플레이어 객체가 소유하며, **계정 단위로 `dbo.Inventory`에
  저장·복원됩니다**(2026-07-26). 계정은 입장 티켓으로 확정됩니다.
  - 입장 시에는 **빈 인벤토리**로 시작하고, DB 로드가 돌아온 뒤 실제 내용이 채워집니다 —
    **클라는 입장 직후 스냅샷을 두 번 받습니다**(빈 것 → 실제 것).
  - 로드가 끝나기 전의 `C_InventoryAction`은 `StaleRevision`으로 거절되고 스냅샷이 재전송됩니다.
    클라는 이미 스냅샷으로 재동기화하므로 별도 처리가 필요 없습니다.
  - DB 장애로 로드에 실패하면 인벤토리는 빈 채로 남고 **저장도 하지 않습니다**(저장된 내용을
    지워버리지 않기 위해). 전투와 진행은 정상입니다.
  - 서버 상세: `RoomServer/docs/inventoryPersistence.md`
- 시작 아이템은 슬롯 0의 체력 회복 물약 5개이며, **계정이 처음 게임에 입장할 때 1회만**
  지급되고 곧바로 DB에 저장됩니다. 이후 재입장에는 저장된 내용을 그대로 불러옵니다
  (다 쓰면 아이템 획득 경로가 생기기 전까지 보충되지 않습니다).
...
- `Inventory::add()`는 기존 스택 우선이라는 결정적 순서를 전제로 합니다.
  ...
  있습니다.
- 저장·로드는 구현되었으나(`dbo.Inventory`) **획득 경로가 아직 없습니다.** 루팅·줍기 패킷이
  없어서 인벤토리는 사용·버리기로 줄어들기만 합니다. 월드 드롭, 교환, 무게, 카테고리도
  구현되어 있지 않습니다.
