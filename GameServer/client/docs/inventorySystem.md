...
- 온라인 인벤토리는 `RoomServer`의 플레이어 객체가 소유하며, **계정 단위로 `dbo.Inventory`에
  ...
  - 입장은 DB를 기다리지 않습니다. 먼저 시작 아이템으로 채워 스냅샷을 보내고, DB 로드가
    돌아오면 교체해 스냅샷을 한 번 더 보냅니다 — **클라는 입장 직후 스냅샷을 두 번 받습니다.**
  - 로드가 끝나기 전의 `C_InventoryAction`은 `StaleRevision`으로 거절되고 스냅샷이 재전송됩니다.
    클라는 이미 스냅샷으로 재동기화하므로 별도 처리가 필요 없습니다.
  - DB 장애로 로드에 실패하면 시작 아이템으로 플레이는 계속되지만 **저장은 하지 않습니다**
    (진짜 인벤토리를 덮어쓰지 않기 위해).
  - 서버 상세: `RoomServer/docs/inventoryPersistence.md`
- 시작 아이템은 슬롯 0의 체력 회복 물약 5개입니다(저장된 인벤토리가 없는 신규 계정 한정).
