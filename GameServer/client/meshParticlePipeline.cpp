...
void Dispatcher::updateGPUDataSingleThreaded() {
    ...
    static auto perInstanceData = std::vector<MeshParticleShader::PerInstanceData>();
    ...
    auto pfd = MeshParticleShader::PerFrameData{
...
void Dispatcher::drawSingleThreaded() {
    ...
    DISPLAY_ERROR_STR(cmdListPool_->allocOne(CommandListUsage::RenderingSlave, cmdCtx),
        "[GFX Error] MeshParticlePipeline::drawSingleThreaded: failed to alloc command list", false);
    ...
    for (const auto& e : drawEvents_) {
        ...
        if (e.pMesh->vbViewsByPipeline.find("MeshParticlePipeline") == e.pMesh->vbViewsByPipeline.end()) {
            auto& views = e.pMesh->vbViewsByPipeline["MeshParticlePipeline"];
            views.resize(2u);
