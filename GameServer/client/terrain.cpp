...
ChunkIndex parseChunkIndex(const std::filesystem::path& terrainDir) {
    ...
    for (int i = 0; i < L; ++i) {
        ...
        const auto tileSize   = readVec2(ifs, "TileSize");
        const auto tileOffset = readVec2(ifs, "TileOffset");
        ly.tileSizeX   = tileSize.x;
        ly.tileSizeY   = tileSize.y;
        ly.tileOffsetX = tileOffset.x;
        ly.tileOffsetY = tileOffset.y;
