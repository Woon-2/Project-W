...
namespace UIPipeline {

	namespace Detail {
		std::vector<ComPtr<ID3D12Resource>> staticVBQuads;
		std::vector<D3D12_VERTEX_BUFFER_VIEW> staticVBViewQuads;
		std::map<std::string, u32t> staticVBIdxMapQuads;
		ComPtr<ID3D12Resource> staticIBQuad;
		D3D12_INDEX_BUFFER_VIEW staticIBViewQuad;
	}	// namespace UIPipeline::Detail
	...
	void initStaticQuadMesh( ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, Fence& fenceToAssociate ) {
		...
		setD3DName( vbPosition.Get(), "QuadMesh_VB_Position" );
		...
		setD3DName( vbPositionu.Get(), "QuadMesh_VB_Position_Upload" );
		...
		// SubMesh 구성
		Detail::staticIBViewQuad = D3D12_INDEX_BUFFER_VIEW{
		...
		Detail::staticVBQuads.push_back( std::move( vbPosition ) );
		Detail::staticVBIdxMapQuads.try_emplace( "PointMesh_VB_Position", 0u );
		Detail::staticVBQuads.push_back( std::move( vbUv ) );
		Detail::staticVBIdxMapQuads.try_emplace( "PointMesh_VB_Size", 1u );
		Detail::staticIBQuad = std::move( ib );
	...
	void Dispatcher::updateGPUDataSingleThreaded() {
		...
		static auto perInstanceData = std::vector<UIShader::PerInstanceData>();
	...
	void MU_CALLCONV Dispatcher::addJobUpdate( const DrawEvent* pFirst,
		const DrawEvent* pLast, UIShader::PerInstanceData* pOut, std::latch& latch
	) {
