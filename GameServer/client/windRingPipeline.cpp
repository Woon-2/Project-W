...
void Dispatcher::updateGPUDataSingleThreaded() {
    ...
    static auto perInstanceData = std::vector<WindRingShader::PerInstanceData>();
    perInstanceData.resize(drawEvents_.size());

    std::ranges::transform(drawEvents_, perInstanceData.begin(),
        [](const DrawEvent& e) {
            return WindRingShader::PerInstanceData{
    ...
    auto pfd = WindRingShader::PerFrameData{
...
void Dispatcher::drawSingleThreaded() {
    ...
    DISPLAY_ERROR_STR(cmdListPool_->allocOne(CommandListUsage::RenderingSlave, cmdCtx),
        "[GFX Error] WindRingPipeline::drawSingleThreaded: failed to alloc command list", false);
    ...
    const auto camPos = cameraData_.pos;
    u32t idxDrawcall = 0u;

    for (const auto& e : drawEvents_) {
        if (!e.pMesh || !e.pSubMesh || !e.pTex) { ++idxDrawcall; continue; }

        // Lazily build pipeline-specific VB view array: Position(0), Normal(1), UV(2).
        if (e.pMesh->vbViewsByPipeline.find("WindRingPipeline") == e.pMesh->vbViewsByPipeline.end()) {
            auto& views = e.pMesh->vbViewsByPipeline["WindRingPipeline"];
            views.resize(3u);
        ...
        const auto& vbViews = e.pMesh->vbViewsByPipeline.at("WindRingPipeline");

        pResources_->perDrawcallData.cbuffers[idxDrawcall].bind(cmdList, rootParamIdxPDD_, roomIdx_);

        auto pdd = WindRingShader::PerDrawcallData{
            .idxTex              = e.pTex->idxSrv,
            .firstInstanceOffset = idxDrawcall,
            .edgeFadePower       = e.edgeFadePower,
            .edgeFadeStrength    = e.edgeFadeStrength,
            .pad0                = 0.f,
            .cameraPosW          = XMFLOAT3{ camPos.x(), camPos.y(), camPos.z() },
            .pad1                = 0.f,
        };
