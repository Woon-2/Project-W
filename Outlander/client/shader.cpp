...
// Energy orb point-sprite shader. Renders into SceneColorHDR (R16G16B16A16_FLOAT)
// with additive blending so bloom makes the orbs glow. Each source vertex is a
// point, GS-expanded to a camera-facing quad. Reversed-Z depth test (GREATER) with
// no depth write, so orbs are occluded by scene geometry but never write depth.
ComPtr<ID3D12PipelineState> createEnergyOrbShader( ID3D12Device* device, ID3D12RootSignature* rootSig ) {
	ComPtr<ID3D12PipelineState> ret{};

	auto vsCode = compileShader( "energyOrb.hlsl", nullptr, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );
	auto psCode = compileShader( "energyOrb.hlsl", nullptr, "PSMain", "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );
	auto gsCode = compileShader( "energyOrb.hlsl", nullptr, "GSMain", "gs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );

	// Slot0=POSITION(float3), Slot1=BONE_INDICES(int4), Slot2=BONE_WEIGHTS(float4), Slot3=UV(float2).
	auto elemDescs = std::vector<D3D12_INPUT_ELEMENT_DESC>{
		D3D12_INPUT_ELEMENT_DESC{
			.SemanticName = "POSITION",
			.SemanticIndex = 0u,
	...
	auto psoDesc = D3D12_GRAPHICS_PIPELINE_STATE_DESC{
		...
		// Reversed-Z: depth-test only (GREATER), no depth write.
		.DepthStencilState = D3D12_DEPTH_STENCIL_DESC{
			.DepthEnable = true,
			.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO,
			.DepthFunc = D3D12_COMPARISON_FUNC_GREATER,
	...
	psoDesc.BlendState.RenderTarget[0].BlendOpAlpha = D3D12_BLEND_OP_ADD;
	psoDesc.BlendState.RenderTarget[0].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL;
	psoDesc.RTVFormats[0] = DXGI_FORMAT_R16G16B16A16_FLOAT;  // SceneColorHDR
	psoDesc.DSVFormat = DXGI_FORMAT_D32_FLOAT;
	...
	setD3DName( ret.Get(), "EnergyOrbShader" );
...
ComPtr<ID3D12PipelineState> createWindRingShader( ID3D12Device* device, ID3D12RootSignature* rootSig ) {
	...
	auto vsCode = compileShader( "windRing.hlsl", nullptr, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );
	auto psCode = compileShader( "windRing.hlsl", nullptr, "PSMain", "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );

	// Slot0=POSITION(float3), Slot1=NORMAL(float3), Slot2=UV(float2)
	auto elemDescs = std::vector<D3D12_INPUT_ELEMENT_DESC>{
		D3D12_INPUT_ELEMENT_DESC{
			.SemanticName = "POSITION",
			.SemanticIndex = 0u,
	...
	setD3DName( ret.Get(), "WindRingShader" );
...
ComPtr<ID3D12PipelineState> createPiercingMeshShader( ID3D12Device* device, ID3D12RootSignature* rootSig ) {
	...
	auto vsCode = compileShader( "piercing.hlsl", nullptr, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );
	auto psCode = compileShader( "piercing.hlsl", nullptr, "PSMain", "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );
	...
	setD3DName( ret.Get(), "PiercingMeshShader" );
...
ComPtr<ID3D12PipelineState> createPiercingSlashMeshShader( ID3D12Device* device, ID3D12RootSignature* rootSig ) {
	...
	auto vsCode = compileShader( "piercingSlash.hlsl", nullptr, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );
	auto psCode = compileShader( "piercingSlash.hlsl", nullptr, "PSMain", "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );
	...
	setD3DName( ret.Get(), "PiercingSlashMeshShader" );
...
// Inverted-hull silhouette. Front faces are culled so only the back of the
// expanded hull survives, forming a rim around the original mesh. Additive into
// SceneColorHDR (R16G16B16A16_FLOAT) before bloom, so an HDR color glows.
// Depth test uses GREATER to match the reversed-Z main camera (near=1/far=0);
// depth write is off so the silhouette never occludes anything.
ComPtr<ID3D12PipelineState> createOutlineShader( ID3D12Device* device, ID3D12RootSignature* rootSig ) {
	ComPtr<ID3D12PipelineState> ret{};

	auto vsCode = compileShader( "outline.hlsl", nullptr, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );
	auto psCode = compileShader( "outline.hlsl", nullptr, "PSMain", "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u );

	// Input layout: Slot0=POSITION(float3), Slot1=NORMAL(float3)
	auto elemDescs = std::vector<D3D12_INPUT_ELEMENT_DESC>{
		D3D12_INPUT_ELEMENT_DESC{
			.SemanticName = "POSITION",
			.SemanticIndex = 0u,
	...
	auto psoDesc = D3D12_GRAPHICS_PIPELINE_STATE_DESC{
		...
		.RasterizerState = D3D12_RASTERIZER_DESC{
			.FillMode = D3D12_FILL_MODE_SOLID,
			.CullMode = D3D12_CULL_MODE_FRONT,   // inverted hull
			.FrontCounterClockwise = false,
			.DepthBias = 0,
			.DepthBiasClamp = 0.f,
			.SlopeScaledDepthBias = 0.f,
		...
		.DepthStencilState = D3D12_DEPTH_STENCIL_DESC{
			.DepthEnable = true,
			.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO,
			.DepthFunc = D3D12_COMPARISON_FUNC_GREATER,   // reversed-Z
			.StencilEnable = false,
			.StencilReadMask = 0u,
			.StencilWriteMask = 0u,
			.FrontFace = D3D12_DEPTH_STENCILOP_DESC{},
	...
	setD3DName( ret.Get(), "OutlineShader" );
...
ComPtr<ID3D12PipelineState> createPBRDeferredIndirectGBufferShader(ID3D12Device* device, ID3D12RootSignature* rootSig) {
	...
	// Same as PBRDeferredGBufferShader but the VS remaps SV_InstanceID through the
	// Hi-Z compacted visible-index list (HiZCull define). Used by the indirect draw path.
	static const D3D_SHADER_MACRO hiZCullDefines[] = { { "HiZCull", "1" }, { nullptr, nullptr } };
	auto vsCode = compileShader("pbrDeferred.hlsl", hiZCullDefines, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u);
	auto psCode = compileShader("pbrDeferred.hlsl", hiZCullDefines, "PSMain", "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u);
	...
	setD3DName(ret.Get(), "PBRDeferredIndirectGBufferShader");
	return ret;
}
...
ComPtr<ID3D12PipelineState> createTonemapResolveShader(ID3D12Device* device, ID3D12RootSignature* rootSig) {
	...
	auto vsCode = compileShader("tonemapResolve.hlsl", nullptr, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u);
	auto psCode = compileShader("tonemapResolve.hlsl", nullptr, "PSMain", "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u);
	...
	auto psoDesc = D3D12_GRAPHICS_PIPELINE_STATE_DESC{
		...
		.DepthStencilState = D3D12_DEPTH_STENCIL_DESC{
			.DepthEnable      = false,                        // no depth test/write in resolve pass
			.DepthWriteMask   = D3D12_DEPTH_WRITE_MASK_ZERO,
			.DepthFunc        = D3D12_COMPARISON_FUNC_ALWAYS,
			.StencilEnable    = false,
			.StencilReadMask  = 0u,
	...
	setD3DName(ret.Get(), "TonemapResolveShader");
	return ret;
}

// Shared builder for the three bloom PSOs. All render to an RGBA16F bloom mip with
// no depth and no culling (fullscreen triangle). `additive` enables ONE/ONE blending
// for the upsample pass (accumulates the smaller mip onto the larger one).
static ComPtr<ID3D12PipelineState> createBloomShaderImpl(
	ID3D12Device* device, ID3D12RootSignature* rootSig,
	const char* psEntry, bool additive, const char* name
) {
	ComPtr<ID3D12PipelineState> ret{};

	auto vsCode = compileShader("bloom.hlsl", nullptr, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u);
	auto psCode = compileShader("bloom.hlsl", nullptr, psEntry,  "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u);
	...
	psoDesc.NumRenderTargets = 1u;
	psoDesc.BlendState.RenderTarget[0].BlendEnable           = additive;
	psoDesc.BlendState.RenderTarget[0].SrcBlend              = D3D12_BLEND_ONE;
	psoDesc.BlendState.RenderTarget[0].DestBlend             = additive ? D3D12_BLEND_ONE : D3D12_BLEND_ZERO;
	psoDesc.BlendState.RenderTarget[0].BlendOp               = D3D12_BLEND_OP_ADD;
	psoDesc.BlendState.RenderTarget[0].SrcBlendAlpha         = D3D12_BLEND_ONE;
	psoDesc.BlendState.RenderTarget[0].DestBlendAlpha        = D3D12_BLEND_ZERO;
	psoDesc.BlendState.RenderTarget[0].BlendOpAlpha          = D3D12_BLEND_OP_ADD;
...
ComPtr<ID3D12PipelineState> createBloomPrefilterShader(ID3D12Device* device, ID3D12RootSignature* rootSig) {
	return createBloomShaderImpl(device, rootSig, "PSPrefilter", false, "BloomPrefilterShader");
}
ComPtr<ID3D12PipelineState> createBloomDownsampleShader(ID3D12Device* device, ID3D12RootSignature* rootSig) {
	return createBloomShaderImpl(device, rootSig, "PSDownsample", false, "BloomDownsampleShader");
}
...
// Heat-haze glow pass PSO. Fullscreen triangle, no depth, additive ONE/ONE blend
// into the HDR scene-color RT (so the tint blooms). Mirrors the bloom upsample PSO
// but targets R16G16B16A16_FLOAT and has no vertex input.
ComPtr<ID3D12PipelineState> createHeatHazeShader(ID3D12Device* device, ID3D12RootSignature* rootSig) {
	ComPtr<ID3D12PipelineState> ret{};

	auto vsCode = compileShader("heatHaze.hlsl", nullptr, "VSMain", "vs_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u);
	auto psCode = compileShader("heatHaze.hlsl", nullptr, "PSMain", "ps_6_0", D3DCOMPILE_ENABLE_UNBOUNDED_DESCRIPTOR_TABLES, 0u);
	...
	psoDesc.NumRenderTargets = 1u;
	psoDesc.BlendState.RenderTarget[0].BlendEnable           = true;       // additive into SceneColorHDR
	psoDesc.BlendState.RenderTarget[0].SrcBlend              = D3D12_BLEND_ONE;
	psoDesc.BlendState.RenderTarget[0].DestBlend             = D3D12_BLEND_ONE;
	psoDesc.BlendState.RenderTarget[0].BlendOp               = D3D12_BLEND_OP_ADD;
	psoDesc.BlendState.RenderTarget[0].SrcBlendAlpha         = D3D12_BLEND_ONE;
	...
	setD3DName(ret.Get(), "HeatHazeShader");
	return ret;
}
