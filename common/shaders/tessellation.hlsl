...
HSConstantOutput HSConstant(InputPatch<VSOutput, 4> input) 
{
    ...
    float TessLevel0 = min( lerp( MAX_TESS_LEVEL, MIN_TESS_LEVEL, min(Distance10, Distance00) ), 5.f );
    float TessLevel1 = min( lerp( MAX_TESS_LEVEL, MIN_TESS_LEVEL, min(Distance00, Distance01) ), 5.f );
    float TessLevel2 = min( lerp( MAX_TESS_LEVEL, MIN_TESS_LEVEL, min(Distance01, Distance11) ), 5.f );
    float TessLevel3 = min( lerp( MAX_TESS_LEVEL, MIN_TESS_LEVEL, min(Distance11, Distance10) ), 5.f );
...
DSOutput DSMain(HSConstantOutput input, float2 uv : SV_DomainLocation, const OutputPatch<HSOutput, 4> patch) 
{
	...
	float4 heightPixel = sampleLevelFromMapRef(heightMapRef, output.texcoord, 0.f, heightMapSamplerIdx);
    ...
    float4 tHeightPixel = sampleLevelFromMapRef2DOffset(heightMapRef, output.texcoord, 0.f, int2(0, 1), heightMapSamplerIdx);
    float tHeight = tHeightPixel.a / 16777216.f
