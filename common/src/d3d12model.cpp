...
Model::Model( d3d12::Core& core, d3d12::D3D12RenderContext& ctx, const gfx::Model& model,
...
) : meshes_(), children_(), name_(model.name()), pVbSerialIdx_(pVbSerialIdx), pIbSerialIdx_(pIbSerialIdx) {
    ...
    for (const auto& child : model.children()) {
        children_.push_back(Model(core, ctx, child, vbUpIdx, ibUpIdx, pVbSerialIdx_, pIbSerialIdx_));
    }
}
