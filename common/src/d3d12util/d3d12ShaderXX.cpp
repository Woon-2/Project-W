...
ShaderCube::ShaderCube(D3D12Device &device, const RootSignature &root)
	: Shader(root, makeInputLayout()),
	cbDrawcallDataSize_( calcConstantBufferSize(sizeof(sr::PerDrawcallData1)) ),
	perDrawcallData_(device, cbDrawcallDataSize_) {
	perDrawcallData_.pullGpuAddr();
}

void ShaderCube::bindRootParams(D3D12GfxCmdList& cmdList) {
	cmdList.get()->SetGraphicsRootConstantBufferView(
		UnifiedRoot::get().params[ UnifiedRoot::ParamIndices::b0 ],
		perDrawcallData_.gpuAddr()
	);
}

void ShaderCube::loadBlobs() {
	blobs_[etoi(ShaderBlob::Type::Vertex)] = ShaderBlob{
		shaderPath/"cube.hlsl", inputLayout(), nullptr,
		"VSMain", "vs_5_1", 0, 0, ShaderBlob::Type::Vertex
	};
	blobs_[etoi(ShaderBlob::Type::Pixel)] = ShaderBlob{
		shaderPath/"cube.hlsl", inputLayout(), nullptr,
...
void ShaderCube::releaseBlobs() {
	blobs_[etoi(ShaderBlob::Type::Vertex)].reset();
	blobs_[etoi(ShaderBlob::Type::Pixel)].reset();
}

InputLayout ShaderCube::makeInputLayout() {
	return InputLayout( std::vector<InputLayout::Slot>{
		InputLayout::Slot{
			.elems = {
				InputLayout::Elem{ .semanticName = "POSITION", .semanticIndex = 0u, .format = DXGI_FORMAT_R32G32B32_FLOAT }
