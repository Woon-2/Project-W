...
void BasicPlayerHpUI::update( Milliseconds deltaTime, GFX& gfx, FontHandle* pFontHandle ) {
	mu::Vec3 scale{ scale_.x() * 0.5f * ( static_cast<float>(hp_) * 0.01f ), scale_.y() * 0.5f, 1.0f };
	mu::Vec3 translation{ pivot_.x() - scale_.x() * 0.5f * ( static_cast<float>(100 - hp_) * 0.01f ), pivot_.y(), 0.f };
