...
// @brief result 값이 false로 평가되면 std::cout을 통해,
	...
	{	\
		...
		if (__dp_e_hr_result < 0) {	\
			...
			if (!DXGIDebugInfo::infoQ) {	\
				std::cout << "[DXGIDebugInfo] DXGIDebugInfo::infoQ가 활성화되지 않았습니다. "	\
					"DXGI에서 발생한 자세한 예외의 정보를 확인할 수 없습니다.\n";	\
			}	\
			else {	\
...
#define DISPLAY_ERROR_DX_VOID(voidCall, willExit)	\
	{	\
		if (!DXGIDebugInfo::infoQ) {	\
			std::cout << "[DXGIDebugInfo] DXGIDebugInfo::infoQ가 활성화되지 않았습니다. "	\
				"DISPLAY_ERROR_DX_VOID로 예외를 감지할 수 없습니다."	\
				", from file " << __FILE__ << ", line " << __LINE__ << '\n';	\
			(voidCall);	\
		}	\
		else {	\
...
// @param hr 실행된 함수의 반환값(HRESULT 타입)
// @param willExit 예외 발생 시 종료 여부
// @brief hr 값이 false로 평가되지 않으면 std::cout을 통해,
// hr과 함께 hr이 나타내는 예외 내용을 해석한 자연어 문장을 출력한다.
