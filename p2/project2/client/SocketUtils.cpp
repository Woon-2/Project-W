...
SOCKET SocketUtils::createSocket( ) {
	auto sock = ::WSASocket( AF_INET, SOCK_STREAM, IPPROTO_TCP,
...
void SocketUtils::bind( SOCKET sock, const NetAddress& addr ) {
	ASSERT_CRASH( ::bind( sock,
		reinterpret_cast<const SOCKADDR*>( &addr.getSockAddr( ) ),
		sizeof( SOCKADDR_IN ) ) != SOCKET_ERROR );
}

void SocketUtils::bindAnyAddr( SOCKET sock, uint16 port ) {
	auto addr = NetAddress( port );
	ASSERT_CRASH( ::bind( sock,
		reinterpret_cast<const SOCKADDR*>( &addr.getSockAddr( ) ),
		sizeof( SOCKADDR_IN ) ) != SOCKET_ERROR );
}
